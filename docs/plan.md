...
- **Jev:** integrado como experimento sintético con AI SDK y como proveedor opcional de FastAPI. Kev sigue predeterminado; Jev solo se elige con `VIGILIA_AI_PROVIDER=jev`, exige `AI_GATEWAY_API_KEY` y ZDR para datos del flujo real, limita la ruta a `typesafe-ai`, minimiza el estado enviado y falla cerrada si el Gateway no confirma la ruta y ZDR en sus metadatos. Vercel documenta soporte de ZDR y No Training para Jev; su tabla pública de proveedor no marca actualmente esos atributos, así que hay una discrepancia entre fuentes oficiales. Las pruebas de esta cuenta no han confirmado ZDR: las solicitudes previas recibieron 403 y cuatro llamadas ZDR sintéticas del 25 de septiembre devolvieron HTTP 502 por errores upstream 500. Otra instancia local obtuvo seis fallos similares en modo No Training, mientras el proceso local ya activo volvió a pasar; la causa de esa diferencia no está aislada. No prueba por sí solo que Jev sea incompatible. El simulador compara dos ejemplos visibles (`vig-demo-01` y `vig-demo-04`) y un fixture de regresión no seleccionable (`vig-demo-07`) con coincidencia textual exacta. Seis llamadas consecutivas pasaron con No Training y mantuvieron estables las categorías pendiente, posible y directa. Un muestreo posterior devolvió probabilidades 0.51 para `PENDIENTE`, 0.99/0.95 para `POSIBLE` y 0.97 para `DIRECTA`; las decisiones se mantienen al aplicar el umbral 0.75, pero esto no mide exactitud ni latencia de producción. No usar Jev con ingresos reales hasta confirmar elegibilidad ZDR y observar una respuesta del backend cuya ruta final y planificación confirmen la política.
...
Antes de habilitar Jev en el backend con ingresos reales, resolver por qué AI Gateway no acepta la solicitud ZDR de esta cuenta. La prueba sintética AI SDK del 25 de septiembre falló con HTTP 502 / `GatewayResponseError` tanto con la lista de proveedor como sin ella; la repetición de cuatro solicitudes desde un servidor temporal volvió a fallar con HTTP 502 y error upstream 500. Las llamadas de control bajo No Training sí completaron. El benchmark local permite repetir la prueba ZDR con fixtures sin enviar datos de usuarios.

La prueba ZDR se repitió desde tres rutas: el AI SDK devolvió HTTP 502 y tanto el endpoint HTTP nativo como el adaptador Python recibieron HTTP 403 con datos ficticios; el backend mantuvo la revisión humana pendiente. La ruta HTTP bajo No Training sí devolvió 200 y confirmó `finalProvider=typesafe-ai`. Vercel exige un equipo Pro o Enterprise para ZDR por solicitud. El estado público del Gateway figuraba operativo durante las pruebas, por lo que falta confirmar el acceso de la cuenta y la elegibilidad ZDR del proveedor, sin asumir una incidencia global.
