...
- **Jev:** integrado como experimento sintético con AI SDK y como proveedor opcional de FastAPI. Kev sigue predeterminado; Jev solo se elige con `VIGILIA_AI_PROVIDER=jev`, exige `AI_GATEWAY_API_KEY` y ZDR para datos del flujo real, minimiza el estado enviado y falla cerrada si el Gateway rechaza la solicitud. El simulador acepta solo `vig-demo-01` y `vig-demo-04`; seis llamadas recientes pasaron con validación de umbral, probabilidades, revisión humana y repetibilidad. AI Gateway rechazó ZDR con 403 y su catálogo actual no anuncia ZDR ni No Training para Jev; la opción `disallowPromptTraining` completó llamadas sintéticas pero no demuestra que el proveedor la aplique. No activar Jev con ingresos reales hasta confirmar por contrato y por respuesta enrutada que el servicio admite las salvaguardas necesarias.
...
1. **Confirmar el tratamiento de datos de Jev antes de activarlo en el backend.** Solicitar a Vercel/TypeSafe confirmación contractual de ZDR para Jev y de la ruta/proveedor efectivo; un plan Pro/Enterprise no basta si la ficha del modelo no anuncia esa elegibilidad. Volver a probar el adaptador Python con casos ficticios y verificar metadatos de enrutamiento. No enviar ingresos reales mientras no se confirme.
...
La llamada Jev del simulador está validada técnicamente con seis evaluaciones sintéticas estables que también verifican el umbral configurado y la revisión humana, pero no acreditan exactitud clínica ni habilitan su uso con datos reales. La ruta del proveedor de backend requiere ZDR, que no está anunciado para Jev en el catálogo actual de AI Gateway y fue rechazada en la llamada de prueba.
