...
La integración con AI Gateway se probó con una clave local sin exponerla. El catálogo respondió correctamente y Jev apareció disponible. La llamada con ZDR fue rechazada con `permission_denied`; una llamada con `disallowPromptTraining` sí completó, pero eso no demuestra que el proveedor haya aplicado la política. En el catálogo público actual de Vercel, Jev no tiene marcados los campos ZDR ni No Training. El DPA público de TypeSafe describe retención mientras sea necesaria para prestar el servicio y deja como `N/A` el campo de salvaguardas específicos para datos sensibles. El ZDR por solicitud de Gateway, además, está restringido a equipos Pro y Enterprise; una mejora de plan por sí sola no acredita que la ruta de Jev sea elegible.

El experimento del simulador solo acepta fixtures ficticios y debe seguir aislado del flujo con datos de personas. El agente Python acepta Jev únicamente con ZDR obligatorio y falla cerrada, pero esa ruta no quedó validada porque AI Gateway devolvió 403 y el catálogo no anuncia ZDR para Jev. No habilitarla con datos reales hasta obtener confirmación contractual y técnica del proveedor/ruta efectiva, verificar una respuesta enrutada bajo ZDR y aprobar la evaluación de calidad y privacidad para el uso previsto. Mientras tanto, Kev sigue siendo el proveedor predeterminado del backend.
...
Fuentes oficiales: [modelo Jev en AI Gateway](https://vercel.com/ai-gateway/models/jev), [modelos de TypeSafe y disponibilidad ZDR/No Training](https://vercel.com/ai-gateway/models/providers/typesafe-ai), [DPA de TypeSafe](https://typesafe.ai/legal/data-processing), [guía de evaluación de AI Gateway](https://vercel.com/docs/ai-gateway/modalities/evaluation), [privacidad y ZDR en AI Gateway](https://vercel.com/changelog/zero-data-retention-no-prompt-training-on-ai-gateway), [AI SDK evaluation](https://ai-sdk.dev/docs/ai-sdk-core/evaluation), [descripción de Jev y uso humano](https://vercel.com/i/what-is-jev).
